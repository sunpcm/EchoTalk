...
### Phase 5：双轨制与动态模型配置重构 (架构扩展)

**目标**： 搭建基础/自定义双轨制路由，支持用户按需配置并注入自己的外部大模型/服务密钥。

**后端任务**：

- 在数据库 (`user_settings` 表) 增加 `is_custom_mode` 开关字段与加密密钥存储，并提供配置读写 API 接口。
- 引入加密模块 (`cryptography.fernet`)，确保存储的密钥绝对安全。
- 重构 Agent 工厂逻辑 (`agent.py` 与 `PluginFactory`)，根据双轨状态实施严格的配置隔离和“报错不降级”策略。

**前端任务**：

- 实现状态管理水合 (`useSettingsStore`) 与前后端同步。
